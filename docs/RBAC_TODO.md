...
- [x] Add `Role::can(roles: &[Role]) -> bool` helper on `crate::entity::user::Role` for template button visibility. Registered as a Tera filter named `can` — usage: `{% if current_user.role | can(roles=["Admin","Accountant"]) %}`.
